...
from swagger_server.models.request_assign_order import RequestAssignOrder
...
class LogbookView(MethodView):
    ...
    def post_logbook_entry(self):  # noqa: E501
    ...
    def get_all_categories(self):
        ...
        try:
            if connexion.request.headers:
                ...
                results = self.logbook_use_case.get_all_categories(headers, internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
    ...
    def get_all_authorized(self):
        internal_process = (None, None)
        function_name = "get_all_authorized"
        response = {}
        status_code = 500
        try:
            if connexion.request.headers:
                ...
                message = f"start request: {function_name}, channel: {request.headers.get('channel')}"
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                results = self.logbook_use_case.get_all_authorized(internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
    ...
    def get_all_destiny(self):
        internal_process = (None, None)
        function_name = "get_all_destiny"
        response = {}
        status_code = 500
        try:
            if connexion.request.headers:
                ...
                message = f"start request: {function_name}, channel: {request.headers.get('channel')}"
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                results = self.logbook_use_case.get_all_destiny(request.headers.get('business'), internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
                response["message"] = "Registros obtenidos correctamente"
                response["data"] = results
                end_time = default_timer()
    ...
    def get_group_business_by_sector(self, id_sector):
        internal_process = (None, None)
        function_name = "get_group_business_by_sector"
        response = {}
        status_code = 500
        try:
            if connexion.request.headers:
                ...
                message = f"start request: {function_name}, channel: {request.headers.get('channel')}"
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                result = self.logbook_use_case.get_group_business_by_sector(id_sector, internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
                response["message"] = "Grupos de negocio obtenidos correctamente"
                response["data"] = result
                end_time = default_timer()
    ...
    def get_all_logbooks(self):
        internal_process = (None, None)
        function_name = "get_all_logbooks"
        response = {}
        status_code = 500
        try:
            if connexion.request.headers:
                ...
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                headers = {k.lower(): v for k, v in request.headers.items()}
                result = self.logbook_use_case.get_all_logbooks(headers, request.args, internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
                response["message"] = "Historial de bitacoras obtenido correctamente"
                response["data"] = result
                end_time = default_timer()
    ...
    def get_all_logbooks_paginated(self):
        internal_process = (None, None)
        function_name = "get_all_logbooks_paginated"
        response = {}
        status_code = 500
        try:
            if connexion.request.headers:
                ...
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                headers = {k.lower(): v for k, v in request.headers.items()}
                result = self.logbook_use_case.get_all_logbooks_new(headers, request.args, internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
                response["message"] = "Historial de bitacoras obtenido correctamente"
                response["data"] = result
                end_time = default_timer()
    ...
    def get_leads(self):
        ...
        try:
            if connexion.request.headers:
                ...
                message = f"start request: {function_name}, channel: {request.headers.get('channel')}"
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                results = self.logbook_use_case.get_leads(internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
                response["message"] = "Registros obtenidos correctamente"
                response["data"] = results
                end_time = default_timer()
    ...
    def get_order_receipts(self):
        ...
        try:
            if connexion.request.headers:
                ...
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                headers = {k.lower(): v for k, v in request.headers.items()}
                result = self.logbook_use_case.get_order_receipts(headers, request.args, internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
                response["message"] = "Registros obtenidos correctamente"
                response["data"] = result
                end_time = default_timer()
    ...
    def import_orders(self):
        ...
        try:
            if request.content_type.startswith("multipart/form-data"):
                ...
                if not excel_file:
                    raise CustomAPIException("Campo file no enviado", 400)

                user = request.form.get("user")

                total = self.logbook_use_case.import_orders(
                    excel_file, user, internal_transaction_id, external_transaction_id
                )
                response["error_code"] = 0
                response["message"] = f"Se importaron {total} ordenes de compra correctamente"
                end_time = default_timer()
                logger.info(f"Fin de la transacción, procesada en : {end_time - start_time} milisegundos",
                            internal=internal_transaction_id, external=external_transaction_id)
                status_code = 200
    ...
    def get_reason_restricition(self):
        internal_process = (None, None)
        function_name = "get_reason_restricition"
        response = {}
        status_code = 500
        try:
            if connexion.request.headers:
                ...
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                headers = {k.lower(): v for k, v in request.headers.items()}
                result = self.logbook_use_case.get_reason_restricition(headers, request.args, internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
                response["message"] = "Registros obtenidos correctamente"
                response["data"] = result
                end_time = default_timer()
    ...
    def graphs_blacklist_balanced(self):
        internal_process = (None, None)
        function_name = "graphs_blacklist_balanced"
        response = {}
        status_code = 500
        try:
            if connexion.request.headers:
                ...
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                headers = {k.lower(): v for k, v in request.headers.items()}
                result = self.logbook_use_case.graphs_blacklist_balanced(headers, request.args, internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
                response["message"] = "Datos obtenidos correctamente"
                response["data"] = result
                end_time = default_timer()
    ...
    def delete_blacklist(self):
        internal_process = (None, None)
        function_name = "delete_blacklist"
        response = {}
        status_code = 500
        try:
            if connexion.request.headers:
                ...
                message = f"start request: {function_name}, channel: {request.headers.get('channel')}"
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                self.logbook_use_case.delete_blacklist(request.args, internal_process)
                response["error_code"] = 0
                response["message"] = "Registro de lista negra eliminado correctamente"
                end_time = default_timer()
                logger.info(f"Fin de la transacción, procesada en : {end_time - start_time} milisegundos",
                            internal=internal_transaction_id, external=external_transaction_id)
                status_code = 200
    ...
    def assign_order_to_receipt(self, body=None):  # noqa: E501
        internal_process = (None, None)
        function_name = "assign_order_to_receipt"
        response = {}
        status_code = 500
        try:
            if connexion.request.is_json:
                body = RequestAssignOrder.from_dict(connexion.request.get_json())  # noqa: E501
                start_time = default_timer()
                internal_transaction_id = str(generate_internal_transaction_id())
                external_transaction_id = body.external_transaction_id
                internal_process = (internal_transaction_id, external_transaction_id)
                ...
                message = f"start request: {function_name}, channel: {body.channel}"
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                self.logbook_use_case.assign_order_to_receipt(body.data, internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
                response["message"] = "Orden asignada correctamente"
                end_time = default_timer()
                logger.info(f"Fin de la transacción, procesada en : {end_time - start_time} milisegundos",
                            internal=internal_transaction_id, external=body.external_transaction_id)
                status_code = 200
    ...
    def report_orders(self):
        ...
        try:
            if connexion.request.headers:
                ...
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                headers = {k.lower(): v for k, v in request.headers.items()}
                result = self.logbook_use_case.report_orders(headers, request.args, internal_transaction_id, external_transaction_id)
                output = BytesIO(result["content"])
                output.seek(0)            
                end_time = default_timer()
                logger.info(f"Fin de la transacción, procesada en : {end_time - start_time} milisegundos",
