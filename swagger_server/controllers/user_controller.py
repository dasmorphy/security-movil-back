...
from swagger_server.models.request_form_expo import RequestFormExpo
...
class UserView(MethodView):
    ...
    def post_form_expo(self, body=None):  # noqa: E501
        """Guarda el usuario de ingreso en la base de datos.
        ...
        """
        internal_process = (None, None)
        function_name = "post_form_expo"
        response = {}
        status_code = 500
        try:
            if connexion.request.is_json:
                body = RequestFormExpo.from_dict(connexion.request.get_json())  # noqa: E501
                start_time = default_timer()
                internal_transaction_id = str(generate_internal_transaction_id())
                external_transaction_id = body.external_transaction_id
                internal_process = (internal_transaction_id, external_transaction_id)
                ...
                message = f"start request: {function_name}, channel: {body.channel}"
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                self.user_use_case.form_expo(body.data, internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
    ...
    def login(self, body=None):  # noqa: E501
        ...
        try:
            if connexion.request.is_json:
                ...
                if (body.channel == 'ZENTINEL_WEB' or body.channel == 'ZENTINEL_TEST'):
    ...
    def send_email_qr(self, id_form):
        internal_process = (None, None)
        function_name = "send_email_qr"
        response = {}
        status_code = 500
        try:
            if connexion.request.headers:
                ...
                message = f"start request: {function_name}, channel: {request.headers.get('channel')}"
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                result = self.user_use_case.send_email_qr(id_form, internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
